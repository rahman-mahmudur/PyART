...
with open('traincsv-1/total_data.csv','w+') as f:
	f.write(total_data)		
with open('traincsv-1/total_label.csv','w+') as f:
	f.write(total_label)


start=time.time()
data=pd.read_csv('traincsv-1/total_data.csv')
label=pd.read_csv('traincsv-1/total_label.csv')
lines=len(data)
trainnum=lines
...
joblib.dump(clf,'traincsv-1/total3.pkl')
print(result)
#sys.exit()
end=time.time()
print(end-start)
